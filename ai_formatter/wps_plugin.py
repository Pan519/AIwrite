...
class WPSPlugin:
    ...
    def init_wps(self):
        """初始化WPS应用"""
        try:
            self.wps_app = win32com.client.Dispatch("KingsoftOffice.Application")
            self.wps_app.Visible = True
        except Exception as e:
            messagebox.showerror("错误", f"无法启动WPS: {e}")
            self.wps_app = None
    ...
    def get_current_document(self):
        """获取当前文档"""
        if self.wps_app and self.wps_app.ActiveDocument:
            self.current_document = self.wps_app.ActiveDocument
    ...
    def save_document_as_temp(self, doc):
        ...
        temp_file = temp_dir / "wps_plugin_temp.docx"
    ...
    def load_document_from_temp(self, temp_file, doc):
        ...
        try:
            ...
            doc = self.wps_app.Documents.Open(temp_file)
            return doc
        except Exception as e:
    ...
    def add_menu_item(self):
        ...
        try:
            # 添加菜单项
            menu_bar = self.wps_app.CommandBars("Menu Bar")
